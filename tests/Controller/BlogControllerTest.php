...
class BlogControllerTest extends WebTestCase
{
    public function testIndex()
    {
        ...
        $this->assertCount(
            Manga::NUM_ITEMS,
            $crawler->filter('article.manga'),
            'The homepage displays the right number of mangas.'
        );
    }
    ...
    public function testRss()
    {
        ...
        $this->assertCount(
            Manga::NUM_ITEMS,
            $crawler->filter('item'),
            'The xml file displays the right number of mangas.'
        );
    }
    ...
    public function testNewComment()
    {
        ...
        // Find first blog manga
        $crawler = $client->request('GET', '/en/blog/');
        $mangaLink = $crawler->filter('article.manga > h2 a')->link();

        $crawler = $client->click($mangaLink);
        ...
        $newComment = $crawler->filter('.manga-comment')->first()->filter('div > p')->text();
