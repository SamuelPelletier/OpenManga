...
class DefaultControllerTest extends WebTestCase
{
     ...
     * blog manga fixtures are randomly generated and there's no guarantee that
     * a given blog manga slug will be available.
     */
    public function testPublicBlogManga()
    {
        $client = static::createClient();
        // the service container is always available via the test client
        $blogManga = $client->getContainer()->get('doctrine')->getRepository(Manga::class)->find(1);
        $client->request('GET', sprintf('/en/blog/mangas/%s', $blogManga->getSlug()));
    ...
    {
        yield ['/en/admin/manga/'];
        yield ['/en/admin/manga/new'];
        yield ['/en/admin/manga/1'];
        yield ['/en/admin/manga/1/edit'];
    }
}
